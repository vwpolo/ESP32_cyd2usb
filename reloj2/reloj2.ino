...
#include "min.c"   //10x100 px; eje del reloj en 4x94
#include "hour.c"  //7x75 px; eje del reloj en 3x71
...
float lat = 41.65606;
float lon = -0.87734;
//String url="https://api.openweathermap.org/data/2.5/weather?lat=" +String(lat,6) + "&lon=" + String(lon,6) + "&units=metric&lang=es&appid=xxe0374";
...
LV_FONT_DECLARE(font_rm_2009_20);
...
void setup() {
  ...
  lv_timer_t * timer3 = lv_timer_create(update_seg, 250,NULL);
...
void update_seg(lv_timer_t * timer3){
  static float angulo_s = 0;

//360 grados entre 250ms, se tiene que mover 1.44 cada 250ms del timer
// para lvgl hay que multiplicar todo por 10, que trabaja en decimas de angulo, 3600 una vuelta completa
   angulo_s += 14.4;
...
lv_obj_set_size(scale1,220,220);
lv_obj_set_style_bg_opa(scale1, LV_OPA_COVER, 0);
lv_color_t c1 = lv_color_make(236, 226, 198);
lv_obj_set_style_bg_color(scale1, c1, 0);
lv_obj_set_style_radius(scale1, LV_RADIUS_CIRCLE, 0);
lv_obj_set_style_clip_corner(scale1, true, 0);
lv_obj_set_style_text_font(scale1, &font_rm_2009_20, 0);
//lv_obj_set_style_translate_x(scale1, 5, LV_PART_INDICATOR);
//lv_obj_center(scale1);
...
lv_scale_set_total_tick_count(scale1,61);
lv_scale_set_major_tick_every(scale1, 5);
...
//  lv_image_set_pivot(punto, 4, 120);
  ...
  lv_obj_set_pos(segundero,110-5,110-96);
  lv_image_set_pivot(segundero,5,96);


  minutero = lv_img_create(reloj);
  lv_img_set_src(minutero, &minu);
  lv_obj_set_pos(minutero,110-5,110-94);
  lv_image_set_pivot(minutero,4,94);
  
  horaria = lv_img_create(reloj);
  lv_img_set_src(horaria, &hour);
  lv_obj_set_pos(horaria,110-3,110-71);
  lv_image_set_pivot(horaria,3,71);
